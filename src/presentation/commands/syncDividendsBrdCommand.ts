...
@injectable()
export default class SyncDividendsBrdCommand implements ICommands {

    private name = "sync-dividends-brd";
    ...
    async execute(requestId: string): Promise<void> {
        ...
        await Promise.all(investments.items.map(async ({id, name}) => {
            try {
                const data = await this.investService.getDividens(ECategoryType.BDR, name);
