...
@injectable()
export default class SyncDividendsAcaoCommand implements ICommands {

    private name = "sync-dividends-acao";
    ...
    async execute(requestId: string): Promise<void> {
        ...
        const investments = await this.coreService.getInvestments({filterBy: [
            `category.name ${EWhereOperator.Equal} ${ECategoryType.ACAO}`,
            `balance ${EWhereOperator.GreaterThan} 0`
        ]}, {requestId});

        await Promise.all(investments.items.map(async ({id, name}) => {
            try {
                const data = await this.investService.getDividens(ECategoryType.ACAO, name);
